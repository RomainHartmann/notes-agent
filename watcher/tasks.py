...
def launch_claude_code(project_path, task_prompt, note_id, item, config, title=""):
    ...
    worktree_dir = os.path.join(tempfile.gettempdir(), f"claude_worktree_{safe_id}_{branch_id}")
    ...
    with open(runner_path, "w") as f:
        ...
        f.write(f'cd "$REPO"\n')
        f.write('git fetch origin 2>/dev/null || true\n')
        ...
        f.write(f'git worktree add "$WORKTREE" -b "$BRANCH" "origin/{dev_branch}" 2>/dev/null || ')
        ...
        f.write('    PR_URL=""\n')
        f.write(f'    if command -v gh &>/dev/null; then\n')
        f.write(f'        PR_URL=$(gh pr create --base "{dev_branch}" --head "$BRANCH" '
                f'--title "{safe_commit_msg}" '
                f'--body "Auto-generated by notes-agent from Apple Notes." 2>/dev/null)\n')
        f.write('    fi\n\n')
        f.write('    cd "$REPO"\n')
        f.write('    git worktree remove "$WORKTREE" --force\n')
        f.write(f'    NOTIF_TITLE="PR created: {safe_title}"\n')
        f.write('    if [ -n "$PR_URL" ]; then\n')
        f.write(f'        NOTIF_MSG="{target} - $PR_URL"\n')
        f.write('    else\n')
        f.write(f'        NOTIF_MSG="{target} - $BRANCH (pushed, gh not available for PR)"\n')
        f.write('    fi\n')
        f.write(f'    curl -s -F "token={pushover_token}" -F "user={pushover_user}" '
                f'-F "title=$NOTIF_TITLE" '
                f'-F "message=$NOTIF_MSG" '
                f'https://api.pushover.net/1/messages.json > /dev/null\n')
        f.write("fi\n\n")
