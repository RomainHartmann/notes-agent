...
def launch_claude_code(project_path, task_prompt, note_id, item, config, title=""):
    ...
    with open(runner_path, "w") as f:
        ...
        f.write(f'LOCKDIR="/tmp/claude_merge_$(basename "$REPO").lock"\n')
        f.write('trap \'rmdir "$LOCKDIR" 2>/dev/null\' EXIT\n\n')
        f.write(f'cd "$REPO"\n')
        ...
        f.write('    while ! mkdir "$LOCKDIR" 2>/dev/null; do sleep 1; done\n')
        f.write('    cd "$REPO"\n')
